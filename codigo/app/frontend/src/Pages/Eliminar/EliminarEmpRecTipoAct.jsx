...
export function EliminarEmpRecTipoAct(){
  ...
  const eliminar = async () => {
      if(!window.confirm(`¿Desvincular "${encontrado.Actividad}" de "${encontrado.Empresa}"?`)) return;
      try {
          await api.delete(`/empresa-actividad/${encontrado.ID}`);
    ...
    <>
      ...
      {encontrado && (
        <div style={{border: '2px solid #d9534f', borderRadius: '4px', padding: '30px', backgroundColor: '#fff5f5'}}>
            ...
            <div className="form-group">
                <label>Actividad Ofrecida: </label>
                <input disabled type="text" value={encontrado.Actividad} className="form-control" style={{width:'100%', padding:'8px', fontWeight:'bold'}} />
            </div>
