...
export function ReportarEmpRecTipoServicio(){
  ...
  const cargar = async () => {
    try {
        ...
        if (res.data.length > 0) {
            ...
            toast.info("No se encontraron asignaciones.");
        }
  ...
  return (
    <>
      ...
      <h1>Reportar Servicios por Empresa</h1>
                        ...
                        {datos.length > 0 ? (
                            datos.map(d => (
                                <tr key={d.ID}>
                                    <td>{d.ID}</td>
                                    <td>{d.Empresa}</td>
                                    <td style={{fontWeight:'bold'}}>{d.Servicio}</td>
                                </tr>
                            ))
                        ) : (
                            <tr><td colSpan="3" className="text-center">No hay asignaciones registradas</td></tr>
                        )}
                    </tbody>
                </table>
            </div>
